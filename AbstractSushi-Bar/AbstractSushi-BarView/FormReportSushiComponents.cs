...
{
    public partial class FormReportSushiComponents : Form
    {
        [Dependency]
        public new IUnityContainer Container { get; set; }
        private readonly ReportLogic logic;
        public FormReportSushiComponents(ReportLogic logic)
        {
            InitializeComponent();
            this.logic = logic;
        }
        private void FormReportSushiComponents_Load(object sender, EventArgs e)
        {
            try
            {
                var dict = logic.GetSushiComponent();
                if (dict != null)
                {
        ...
        private void ButtonSaveToExcel_Click(object sender, EventArgs e)
        {
            using (var dialog = new SaveFileDialog { Filter = "xlsx|*.xlsx" })
            {
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        logic.SaveSushiComponentToExcelFile(new ReportBindingModel
                        {
